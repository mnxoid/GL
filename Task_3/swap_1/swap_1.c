 ...
 *									1 - error ( null pointer ) 
 **/
 unsigned char OutArr ( int *a,  const char *name,  int size ) 
  {
  	if  ( a==NULL )  return 1; 
  	int i; 
  	printf ( "%s={", name ) ; 
...
int main (  ) 
{
	...
	char input; 
	if  ( InputChar ( "MH", &input ) ==1 ) 
	 {
	 	printf ( "Null pointer exception!\n" ) ; 
	 	getchar (  ) ; 
	 	return 0; 
	 }
	int *array1, *array2, retc, i, j; 
	unsigned int size; 
	size=0; 
	retc=0; 
	//--------------------Array Input-----------------
	if  (  input == 'M'  ) 
	 { 
	 	...
	 	for  ( i=0; i<size; i++ ) 
	 	 {
	 	 	printf ( "\nA[%i]=", i+1 ) ; 
	 	 	if  ( InputInt (  ( array1+i ) ,  NOLIM,  0,  0 ) ==1 )  {printf ( "Null pointer exception!\n" ) ; return 0; }
	 	 }
	 	for  ( i=0; i<size; i++ ) 
	 	 {
	 	 	printf ( "\nB[%i]=", i+1 ) ; 
	 	 	if  ( InputInt (  ( array2+i ) ,  NOLIM,  0,  0 ) ==1 )  {printf ( "Null pointer exception!\n" ) ; return 0; }
	 	 }
	 } else {
	 	size=5; 
	 	array1=calloc ( size, sizeof ( int )  ) ; 
	...
	//--------------------Array Output----------------
	if ( OutArr ( array1, "A", size ) ==1 )  {printf ( "Null pointer exception!\n" ) ; return 0; }
	if ( OutArr ( array2, "B", size ) ==1 )  {printf ( "Null pointer exception!\n" ) ; return 0; }
	//--------------------Swap------------------------
	printf ( "Which element would you like to swap? " ) ; 
	if  ( InputInt ( &i, USELIM,  0,  size+1 ) ==1 )  {printf ( "Null pointer exception!\n" ) ; return 0; }
	if  ( Swap ( array1, array2, i-1 ) ==1 )  {printf ( "Null pointer exception!\n" ) ; return 0; }
	//--------------------Array Output----------------
	if ( OutArr ( array1, "A", size ) ==1 )  {printf ( "Null pointer exception!\n" ) ; return 0; }
	if ( OutArr ( array2, "B", size ) ==1 )  {printf ( "Null pointer exception!\n" ) ; return 0; }
	//--------------------Memory Deallocation---------
	...
	if  ( InputInt ( &size, USELIM, 0, 1000 ) ==1 )  {printf ( "Null pointer exception!\n" ) ; return 0; }
	int **matrix; 
	...
	for ( i=0; i<size/2; i++ ) 
	 {
	 	for  ( j=0; j<size; j++ ) 
	 	 {
	 	 	if  ( PSwap ( &matrix[i], &matrix[size-i-1], j ) ==1 )  {printf ( "Null pointer exception!\n" ) ; return 0; }
	 	 }
	 }
	//----------------------------Output----------------------------
	printf("After:\n");
